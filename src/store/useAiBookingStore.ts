...
export function buildFareDetails(flight: UnifiedFlight, fareClass: AiFareClass): AiFareDetails {
  ...
  if (!refundable) excludedFeatures.push('Non-refundable');
  else if (refundFee === 0) includedFeatures.push('Fully refundable');
  else if (refundFee !== null) includedFeatures.push(`Refundable (fee $${refundFee})`);
  else includedFeatures.push('Refundable (fee applies)');
  if (!changeable) excludedFeatures.push('Non-changeable');
  else if (changeFee === 0) includedFeatures.push('Free changes');
  else if (changeFee !== null) includedFeatures.push(`Changes allowed ($${changeFee} + fare diff)`);
  ...
  hydrateCheckoutStore: () => {
    ...
    const fareOption: FareOption = apiFare ?? {
      ...
      baggage: {
        ...
        extraBagFeeUsd: EXTRA_BAG_PRICE,
      },
      policy: {
