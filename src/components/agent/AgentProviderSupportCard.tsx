...
/**
 * AgentProviderSupportCard — Agent Portal
 *
 * Internal-only provider support card shown on the agent booking detail page.
 * Only renders for Duffel bookings with a valid providerOrderId.
 *
 * This component is NEVER rendered in customer-facing routes.
 * Uses agent session token (Bearer auth) instead of admin cookie auth.
 */
...
interface AgentProviderSupportCardProps {
  booking: {
    id: string;
...
export default function AgentProviderSupportCard({ booking }: AgentProviderSupportCardProps) {
  ...
  async function openAssistant() {
    ...
    try {
      const res = await fetch(
        apiUrl(`/api/agent/bookings/${booking.masterBookingReference}/provider-support/duffel-assistant`),
        {
          method: 'POST',
          headers: {
    ...
    <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden mt-6">
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <CtxField label="Status" value={booking.bookingStatus} badge />
          <CtxField label="Provider" value="Duffel" />
        </div>
