...
export default function SearchForm({
  ...
  const filteredAirports = useCallback(
    (query: string) =>
      AIRPORTS.filter(
        (a) =>
  ...
  const handleFlexApply = () => {
    ...
    const params = new URLSearchParams({
      origin: originCode,
      destination: destCode,
      date: flexOutbound.date,
      ...(tripType === 'round_trip' && flexReturn ? { return: flexReturn.date } : {}),
      adults: passengers.adults.toString(),
      children: passengers.children.toString(),
      infants: passengers.infants.toString(),
      cabin: cabinClass,
    ...
    const prefQP = prefs.toQueryParams();
    Object.entries(prefQP).forEach(([k, v]) => { if (v) params.set(k, v); });
      ...
      <div className={cn(
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <div className="flex rounded-xl bg-gray-100 p-1 shrink-0">
            ...
            <button
              ...
              onClick={() => { setDateMode('flexible'); onDateModeChange?.('flexible'); }}
              className={cn(
                'px-4 py-2 rounded-lg text-sm font-semibold transition-all whitespace-nowrap',
        ...
        <div className={cn('grid gap-2', gridCols)}>
          ...
          <div ref={originRef} className="relative">
            <div className="relative">
              ...
              <input
                ...
                value={origin}
                onChange={(e) => { setOrigin(e.target.value); setOriginCode(''); setShowOriginDropdown(true); }}
                onFocus={() => {
                  if (originCode) { setOrigin(''); setOriginCode(''); }
                  setShowOriginDropdown(true);
                }}
                className="w-full pl-11 pr-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-[#0F172A] placeholder-gray-500 text-[15px] font-semibold focus:outline-none focus:border-[#F97316] focus:ring-4 focus:ring-[#F97316]/10 focus:bg-white transition-all shadow-sm"
              />
            </div>
            {showOriginDropdown && origin.length >= 1 && (
              <motion.div
                initial={{ opacity: 0, y: -8 }}
                animate={{ opacity: 1, y: 0 }}
                className="absolute top-full mt-2 left-0 min-w-[320px] bg-white rounded-2xl border border-gray-200/80 p-2 z-[200] shadow-2xl shadow-black/15 ring-1 ring-black/5 max-h-72 overflow-y-auto"
              >
                {filteredAirports(origin).map((airport) => (
                  <button
                    key={airport.code}
                    onClick={() => { setOrigin(`${airport.city} (${airport.code})`); setOriginCode(airport.code); setShowOriginDropdown(false); }}
                    className="w-full text-left px-4 py-3 rounded-xl hover:bg-gray-50 transition-all"
                  >
                    <p className="text-sm font-semibold text-[#0F172A]">{airport.city} <span className="text-[#F97316] font-bold">({airport.code})</span></p>
                    <p className="text-xs text-gray-400 mt-0.5">{airport.name}</p>
                  </button>
                ))}
                {filteredAirports(origin).length === 0 && (
                  <p className="text-sm text-gray-400 text-center py-4">No airports found</p>
                )}
              </motion.div>
            )}
          </div>
          ...
          <div ref={destRef} className="relative">
            <div className="relative">
              ...
              <input
                ...
                value={destination}
                onChange={(e) => { setDestination(e.target.value); setDestCode(''); setShowDestDropdown(true); }}
                onFocus={() => {
                  if (destCode) { setDestination(''); setDestCode(''); }
                  setShowDestDropdown(true);
                }}
                className="w-full pl-11 pr-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-[#0F172A] placeholder-gray-500 text-[15px] font-semibold focus:outline-none focus:border-[#1ABC9C] focus:ring-4 focus:ring-[#1ABC9C]/10 focus:bg-white transition-all shadow-sm"
              />
            </div>
            {showDestDropdown && destination.length >= 1 && (
              <motion.div
                ...
                className="absolute top-full mt-2 left-0 min-w-[320px] bg-white rounded-2xl border border-gray-200/80 p-2 z-[200] shadow-2xl shadow-black/15 ring-1 ring-black/5 max-h-72 overflow-y-auto"
              >
                {filteredAirports(destination).map((airport) => (
                  <button
                    key={airport.code}
                    onClick={() => { setDestination(`${airport.city} (${airport.code})`); setDestCode(airport.code); setShowDestDropdown(false); }}
                    className="w-full text-left px-4 py-3 rounded-xl hover:bg-gray-50 transition-all"
                  >
                    <p className="text-sm font-semibold text-gray-900">{airport.city} <span className="text-blue-500 font-bold">({airport.code})</span></p>
                    <p className="text-xs text-gray-400 mt-0.5">{airport.name}</p>
                  </button>
                ))}
          ...
          <div className="flex items-center">
            {(() => {
              const flexReady = dateMode === 'flexible' && canApplyFlex;
              const specificReady = dateMode === 'specific' && !!originCode && !!destCode && originCode !== destCode;
              const enabled = flexReady || specificReady;
              return (
                <button
                  onClick={dateMode === 'flexible' ? handleFlexApply : handleSearch}
                  disabled={!enabled}
                  className={cn(
              ...
              <>
                <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                  {(() => {
                    ...
                    const bestDate     = sortFlexMonths(passing, 'value')[0]?.date    ?? null;
                    const cheapDate    = sortFlexMonths(passing, 'cheapest')[0]?.date ?? null;
                    const fastDate     = sortFlexMonths(passing, 'fastest')[0]?.date  ?? null;

                    const badgeFor = (m: typeof displayMonths[0]): { label: string; cls: string } | null => {
                      if (!m.price || !m.passesFilter) return null;
                      // Best value / AI Pick (highest priority)
                      if (m.date === bestDate) {
                        return { label: '✨ AI Pick', cls: 'bg-[#1ABC9C] text-white shadow-sm shadow-[#1ABC9C]/20' };
                      }
                      // Cheapest (only if a different month than best value)
                      if (m.date === cheapDate && cheapDate !== bestDate) {
                        return { label: 'Cheapest', cls: 'bg-green-500 text-white shadow-sm' };
                      }
                      // Fastest (only if a different month than above)
                      if (m.date === fastDate && fastDate !== bestDate && fastDate !== cheapDate) {
                      ...
                      <div className="ml-auto flex items-center gap-3">
                        ...
                        {canApplyFlex ? (
                          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-orange-50 border border-orange-200">
                            <Plane className="w-3.5 h-3.5 text-orange-500 -rotate-45 shrink-0" />
                            <span className="text-xs font-bold text-orange-700">Hit Search Flights ↑</span>
                          </div>
                        ) : tripType === 'round_trip' && flexOutbound && !flexReturn ? (
