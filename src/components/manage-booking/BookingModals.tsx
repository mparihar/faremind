      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        /* ── Step: Date selection ── */
          ...
          <div className="space-y-4">
            ...
            {changeSearchError && <div className="flex items-start gap-2 text-amber-400 text-sm bg-amber-400/10 border border-amber-400/20 rounded-lg px-3 py-2.5"><AlertCircle size={14} className="shrink-0 mt-0.5" /><span>{changeSearchError.includes('ReIssueQuote failed') ? changeSearchError.replace(/^.*ReIssueQuote failed:\s*/, '') : changeSearchError.includes('automatically.') ? changeSearchError.replace(/^.*automatically\.\s*/, '') : changeSearchError || 'No exchange options available. Please contact FareMind Support for assistance.'}</span></div>}
            <div className="flex gap-3">
            ...
            <div className="space-y-5">
              <div className="bg-[#1ABC9C]/5 border border-[#1ABC9C]/20 rounded-xl p-4">
                <div className="flex justify-between items-start">
                  <div><p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">FM Ref</p><p className="text-white font-black text-xl">{eticket.bookingReference}</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="space-y-4">
          <div className="bg-[#1ABC9C]/5 border border-[#1ABC9C]/20 rounded-xl p-4">
            <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mb-1">FM Ref</p>
