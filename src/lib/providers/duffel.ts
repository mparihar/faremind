...
export async function searchFlights(params: DuffelSearchParams): Promise<DuffelOffer[]> {
  ...
  const offerRequest = await duffelRequest<{
    ...
    body: {
      slices,
      passengers,
      // Omitting cabin_class entirely makes Duffel return offers for ALL available classes
...
export interface DuffelSeatMapAvailableService {
  id: string;
  total_amount: string;
  total_currency: string;
  maximum_quantity: number;
...
export interface DuffelSeatMapElement {
  type: 'seat' | 'empty' | 'exit_row' | 'lavatory' | 'galley' | 'stairs' | 'bassinet';
  designator?: string;
  available_services?: DuffelSeatMapAvailableService[];
  disclosures?: string[];
}
...
/**
 * Fetch seat maps for a Duffel offer (pre-booking) or order (post-booking).
 * Returns one seat map per flight segment in the offer/order.
 */
export async function getSeatMaps(
  id: string,
  type: 'offer' | 'order' = 'offer',
): Promise<DuffelSeatMap[]> {
  return duffelRequest<DuffelSeatMap[]>({
    method: 'GET',
    path: '/air/seat_maps',
    queryParams: { [`${type}_id`]: id },
    retries: 1,
  });
}
