...
export default function AgentMakePaymentPage() {
  ...
  // Generic "Make a Payment For" category. Agents see all three; a wallet-disabled
  // agent is locked to Wallet Recharge only (to restore their wallet).
  const [category, setCategory] = useState<null | 'BOOKING' | 'WALLET' | 'OTHER'>(null);
  ...
  // Detect a wallet-disabled agent → lock the surface to Wallet Recharge only.
  useEffect(() => {
    if (!sessionToken) return;
    fetch('/api/agent/wallet/recharge', { headers: { Authorization: `Bearer ${sessionToken}` } })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => { if (d?.agent && d.agent.active === false) { setWalletDisabled(true); setCategory('WALLET'); } })
      .catch(() => {});
  }, [sessionToken]);
  ...
  // ── "Make a Payment For" category selector (agent: all three) ──
  if (category === null) {
    const cats = [
      { key: 'BOOKING' as const, title: 'Booking Payment', desc: 'Pay for services or extras on a booking.', icon: Ticket },
      { key: 'WALLET' as const, title: 'Wallet Recharge', desc: 'Top up your prepaid agent wallet.', icon: Wallet },
      { key: 'OTHER' as const, title: 'Other Payment', desc: 'A general payment (e.g. support case or payment request).', icon: HelpCircle },
    ];
    return (
  ...
  if (category === 'WALLET') {
    return (
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-black text-white flex items-center gap-2"><Wallet size={22} className="text-[#1ABC9C]" /> Wallet Recharge</h1>
          <p className="text-slate-500 text-sm mt-0.5">Top up your prepaid agent wallet</p>
          {walletDisabled && <p className="text-red-400 text-xs mt-1 font-semibold">Your account is disabled — recharge your wallet to reactivate. This is the only action available until then.</p>}
        </div>
        <WalletRechargePanel sessionToken={sessionToken || ''} onBack={walletDisabled ? undefined : () => setCategory(null)} />
      </div>
    );
  }
