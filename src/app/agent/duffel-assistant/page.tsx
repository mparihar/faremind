...
/**
 * Duffel Assistant — Standalone Agent Page
 *
 * Accessible from the agent sidebar. Allows agents to:
 * 1. See all their Duffel bookings
 * 2. Select a booking and open the Duffel Assistant for it
 *
 * Internal-only — never exposed to customers.
 * Uses agent session token auth (Bearer) instead of admin cookie auth.
 */
...
export default function AgentDuffelAssistantPage() {
  ...
  const agentFetch = useCallback(async (path: string, opts?: RequestInit) => {
  ...
  const loadBookings = useCallback(async () => {
    ...
    try {
      ...
      const res = await agentFetch(`/api/agent/bookings?${params}`);
      if (res.ok) {
        const data = await res.json();
        // Filter to only show Duffel bookings
        const duffelOnly = (data.bookings ?? []).filter(
          (b: any) => b.primaryProvider?.toLowerCase() === 'duffel',
        );
        setBookings(duffelOnly);
      }
    } catch (err) {
      console.error('[agent/duffel-assistant] Failed to load bookings:', err);
    }
    setLoading(false);
  }, [search, agentFetch]);
  ...
  async function openAssistant() {
    ...
    try {
      const res = await agentFetch(
        `/api/agent/bookings/${selectedBooking.masterBookingReference}/provider-support/duffel-assistant`,
        {
          method: 'POST',
      ...
      // Dynamically load Duffel Assistant script
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">Contact Duffel provider support for your bookings</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by FBR, PNR, or passenger name..."
          ...
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-slate-500">
                ...
                <p className="text-xs mt-1">Your Duffel bookings will appear here</p>
              </div>
            ) : (
