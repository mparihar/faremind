...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Markup removed platform-wide — tiles display the raw provider fare, so the
    // drift check compares against the raw provider price directly.
    const currentPrice = providerPrice;
